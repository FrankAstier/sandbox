...
#ifndef LINEAR_HASH_PS_HPP
#define LINEAR_HASH_PS_HPP
...
/**
 * Linear hash with priority splitting.
 */
template <typename K, typename V>
struct LinearHash_PS {

  LinearHash_PS(size_t n_buckets =100, size_t bucket_size =4)
      : buckets(n_buckets),
        priority(),
        bitmap(n_buckets),
        m(n_buckets),
  ...
  void put(const K& key, const V&value) {
    ...
    if (buckets[idx].size() > b) { // overflow
      ...
      std::make_heap(priority.begin(), priority.end(),
      [&](const size_t& a, const size_t& b){ return buckets[a].size() > buckets[b].size(); });
      size_t to_split = *priority.begin();
      std::cout << "\tOverflow on bucket " << idx << " - Splitting bucket " << to_split << std::endl;
